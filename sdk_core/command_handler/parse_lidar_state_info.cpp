  ...
  for (uint16_t i = 0; i < key_num; ++i) {
    ...
    switch (kv->key) {
      ...
      case static_cast<uint16_t>(kKeyFovCfg0) :
        memcpy(&info.fov_cfg0, &packet.data[off], val_len);
        off += val_len;
        break;
      case static_cast<uint16_t>(kKeyFovCfg1) :
        memcpy(&info.fov_cfg1, &packet.data[off], val_len);
        off += val_len;
        break;
      case static_cast<uint16_t>(kKeyRoiEn) :
        memcpy(&info.fov_en, &packet.data[off], val_len);
        off += val_len;
        break;
